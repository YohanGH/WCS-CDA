...
const CategoriesNavigation: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-4 bg-background border-b border-border">
  ...
  if (error) {
    ...
    return (
      <div className="text-center py-4 bg-background border-b border-border">
        <p className="text-primary">Erreur lors du chargement des catégories.</p>
      </div>
    )
  }

  return (
    <nav className="relative bg-black border-b border-border overflow-x-auto">
      {/* Grid Background */}
        ...
        <div className="flex justify-center items-center space-x-6">
          {categories.map((category) => (
            <Link
              ...
              className="text-foreground hover:text-primary transition-colors duration-300 whitespace-nowrap relative group"
            >
              {category.title}
