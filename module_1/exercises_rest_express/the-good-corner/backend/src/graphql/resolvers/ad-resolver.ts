...
export class AdResolver {
  ...
  async ads(): Promise<Ad[]> {
    ...
    const ads = await adRepository.find({ relations: ['category', 'tags'] }); // Find all ads with their category and tags
  ...
  async ad(@Arg('id', () => Int) id: number): Promise<Ad> { // Get an ad by its id
    const adRepository = dataSource.getRepository(Ad);
    const ad = await adRepository.findOne({ where: { id }, relations: ['category', 'tags'] }); // Find an ad by its id with its category and tags

    if (!ad) { // If the ad is not found, throw an error
  ...
  async createAd(@Arg('data') data: CreateAdInput): Promise<Ad> { // Create an ad
    const adRepository = dataSource.getRepository(Ad);
    ...
    let tags: Tag[] = [];
    if (data.tagIds && data.tagIds.length > 0) { // If tags are provided, get them
      tags = await tagRepository.findBy({ id: In(data.tagIds) });
    }

    const ad = adRepository.create({ // Create an ad
      title: data.title,
  ...
  async generateRandomAd(): Promise<Ad> { // Generate a random ad
    const adRepository = dataSource.getRepository(Ad);
  ...
  async updateAd(
    ...
    if (data.title !== undefined) { // If the title is provided, update the ad
      ad.title = data.title;
    }

    if (data.description !== undefined) { // If the description is provided, update the ad
      ad.description = data.description;
    }

    if (data.categoryId !== undefined) { // If the category is provided, update the ad
      const category = await categoryRepository.findOne({ where: { id: data.categoryId } }); // Find the category
      if (!category) { // If the category is not found, throw an error
        throw new AppError('Category not found', 404, 'NotFoundError');
      }
    ...
    if (data.tagIds !== undefined) { // If tags are provided, update the ad
  ...
  async deleteAd(@Arg('id', () => Int) id: number): Promise<boolean> { // Delete an ad
    const adRepository = dataSource.getRepository(Ad);

    const deleteResult = await adRepository.delete(id);

    if (deleteResult.affected === 0) { // If the ad is not found, throw an error
      throw new AppError('Ad not found', 404, 'NotFoundError');
    }
